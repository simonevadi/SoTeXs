...
def ML_eff_new(ray_energy, ray_calc_eff):
    """This function reads excel file that Andrey Sokolov
    produced. I basically counted the columns and made a dictionary to 
    index them.
    ...
    grating_file = 'ML_eff/ELISA_GR2400_2ord_ML-Cr-C_N60_d4.8nm_MLbGR.dat'
    mirror_file = 'ML_eff/ELISA_GR2400_2ord_ML-Cr-C_N60_d4.8nm_MLbGR.dat'
    grating = pd.read_csv(grating_file, sep='\s+', header=[0, 1])
    mirror = pd.read_csv(mirror_file, sep='\s+', header=[0, 1])    
    grating_eff = np.interp(ray_energy, grating['Energy'].to_numpy().flatten(),
                            grating['Efficiency(GR)'].to_numpy().flatten()) 
    mirror_eff = np.interp(ray_energy, mirror['Energy'].to_numpy().flatten(),
                           mirror['Efficiency(GR)'].to_numpy().flatten()) 

    ray_calc_eff = ray_calc_eff*grating_eff*mirror_eff
    return ray_calc_eff
